...
class SphinxBase(GenericDFTJob):
    ...
    def set_structure_group(self, symmetry_enabled=True, keep_angstrom=False):
        ...
        for i_elem, elm_species in enumerate(
            ...
            for elm_pos, elm_magmon, selective in zip(
                ...
                if self.input_writer._spin_enabled:
    ...
    def set_main_group(self):
        ...
        if self.input["Istep"] is not None:
            ...
            if self.input["dE"] is not None:
                self.input.main["linQN"]["dEnergy"] = str(self.input["dE"] / HARTREE_TO_EV)
            ...
            if self._generic_input["restart_for_band_structure"]:
                self.input.main["scfDiag"].append(self.get_scf_group(keepRhoFixed=True))
            else:
            ...
            if self.executable.version is not None:
                ...
                if vers_num[0] > 2 or (vers_num[0] == 2 and vers_num[1] > 5):
                    self.input.main["evalForces"] = odict([("file", '"relaxHist.sx"')])
            else:
    ...
    def set_kpoints(
        ...
        if scheme == "MP":
            ...
            if mesh is not None:
                self.input.basis["kPoint"]["folding"] = str(list(mesh))
            if center_shift is not None:
                self.input.basis["kPoint"]["coords"] = str(list(center_shift))
        elif scheme == "Line":
    ...
    def write_input(self):
        ...
        if self._generic_input["fix_spin_constraint"]:
            ...
            if self.input.spin not in self.input.groups:
                self.input.groups.append(self.input.spin)
        ...
        self.input_writer.write_group(self.input.guess,
            cwd=self.working_directory, file_name="initalGuess.sx")
        self.input_writer.write_group(self.input.spin,
            cwd=self.working_directory, file_name="spinConstraint.sx")
        self.input_writer.write_main(self.input.main,
            cwd=self.working_directory, file_name="main.sx")

        self.input_writer.write_wrapper(cwd=self.working_directory,
    ...
    def check_setup(self):
        with warnings.catch_warnings(record=True) as w:
            ...
            if (
                not (
                    isinstance(self.input.basis["ekt"], int)
                    or isinstance(self.input.basis["ekt"], float)
                )
                or self.input.basis["ekt"] == 0.2
            ):
                warnings.warn(
            ...
            if not (
                isinstance(self.input.basis["kPoint"]["folding"], list)
                or len(self.input.basis["kPoint"]["folding"]) != 3
            ) or self.input.basis["kPoint"]["folding"] == [4, 4, 4]:
                warnings.warn(
                ...
                if any(self.structure.get_initial_magnetic_moments() != None):
                    warnings.warn(
                        "Number of empty states was not specified. Default: \
                            NIONS*1.5 for magnetic systems. "
                    )
                else:
                    warnings.warn(
                        "Number of empty states was not specified. Default: \
                            NIONS for non-magnetic systems"
                    )
            if len(w) > 0:
                print("WARNING:")
                for ww in w:
    ...
    def validate_ready_to_run(self):
        ...
        if len(self.input.main) == 0:
            raise AssertionError(
                "self.input.main group not initialized; \
    ...
    def check_vasp_potentials():
        return any(
            [os.path.exists(os.path.join(p, 'vasp'))
...
class InputWriter(object):
    ...
    def write_wrapper(self, cwd=None, file_name="input.sx", enable_kjxc=False):
        """
    ...
    def write_main(self, group, cwd, file_name):
        file_name = posixpath.join(cwd, file_name)
        with open(file_name, "w") as f:
            f.write(group.to_sx_str())
